...
Scene* Stage2_Layer::createScene()
{

	auto scene = Scene::create();
	auto layer = Stage2_Layer::create();
...
bool Stage2_Layer::init()
{
	...
	auto itemBox = MenuItemImage::create("ui/itemBox.png", "ui/itemBox.png", CC_CALLBACK_1(Stage2_Layer::ItemUse, this));
	...
	auto menuItem = MenuItemImage::create("menu/config_button.png", "menu/config_button_pressed.png", CC_CALLBACK_1(Stage2_Layer::OpenMenu, this));
	...
	//auto bgLayer1 = new Stage2();//생성자 넣기
	//bgLayer1->stageNum=1;
	...
	this->schedule(schedule_selector(Stage2_Layer::tick));
	return true;
}
void Stage2_Layer::OpenMenu(Ref *p)
{
...
void Stage2_Layer::MakeMapRamdom(Stage2 *nowStage, Stage2 *preStage, int num, bool state)
{
	num--;
	if (num == 0)
	{
...
void Stage2_Layer::updateVelocity1(Vec2 point)
{
...
void Stage2_Layer::updateVelocity2(Vec2 point)
{
	// calculate Angle and length
...
void Stage2_Layer::resetJoystick1()
{
	this->updateVelocity1(kCenter1);
}
void Stage2_Layer::resetJoystick2()
{
	log("2");
...
bool Stage2_Layer::handleLastTouch1()
{
...
bool Stage2_Layer::handleLastTouch2()
{
...
void Stage2_Layer::tick(float dt)
{

	if (player->getPosition().x > 1000 && player->getPosition().x < 1100 && player->getPosition().y > 480) // 오른쪽문
	{
		auto nowPlayerStage = (Stage2*)(player->nowStage);
		auto nextPlayerStage = ((Stage2*)(player->nowStage))->next;
	...
	else if (player->getPosition().x > 400 && player->getPosition().x < 500 && player->getPosition().y > 480) // 왼쪽문
	{
		...
		auto nowPlayerStage = (Stage2*)(player->nowStage);
		auto prevPlayerStage = ((Stage2*)(player->nowStage))->prev;
	...
	if (((Stage2*)(player->nowStage))->stageNum == BOSS_MAP_NUM2) //보스 스테이지
	{
		...
		vector<b2Body*> temp = ((Stage2*)(player->nowStage))->monsterBodyVector;
...
void Stage2_Layer::ItemUse(Ref * p)
{
	log("item click");
	...
	if (player->activeItem == 2 && player->nowMagic > 5)
	{
		...
		this->scheduleOnce(schedule_selector(Stage2_Layer::resetStat), 5);
	...
	if (player->activeItem == 4 && player->nowMagic > 1)
	{
		SimpleAudioEngine::getInstance()->playEffect(ITEM_USE);
		auto nowPlayerStage = (Stage2*)(player->nowStage);
		nowPlayerStage->unschedule(schedule_selector(Stage2::AITick2));
		this->scheduleOnce(schedule_selector(Stage2_Layer::resetStat), 5);
...
void Stage2_Layer::resetStat(float dt)
{
	...
	{
		auto nowPlayerStage = (Stage2*)(player->nowStage);
		nowPlayerStage->schedule(schedule_selector(Stage2::AITick2));

	}
}

void Stage2_Layer::onEnter()
{
	Layer::onEnter();
	SimpleAudioEngine::getInstance()->stopBackgroundMusic(true);
	SimpleAudioEngine::getInstance()->playBackgroundMusic(STAGE2_BGM, true);
	auto listener = EventListenerTouchAllAtOnce::create();
	listener->onTouchesBegan = CC_CALLBACK_2(Stage2_Layer::onTouchesBegan, this);
	listener->onTouchesMoved = CC_CALLBACK_2(Stage2_Layer::onTouchesMoved, this);
	listener->onTouchesEnded = CC_CALLBACK_2(Stage2_Layer::onTouchesEnded, this);
	listener->onTouchesCancelled = CC_CALLBACK_2(Stage2_Layer::onTouchesCancelled, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);
}

//void Stage2_Layer::onExit()
//{
//	
...
void Stage2_Layer::onTouchesBegan(const std::vector<Touch*>& touches, Event  *event)
{
...
void Stage2_Layer::onTouchesMoved(const std::vector<Touch*>& touches, Event  *event)
{
	for (auto &item : touches)
	{
...
void Stage2_Layer::onTouchesEnded(const std::vector<Touch*>& touches, Event  *event)
{
	for (auto &item : touches)
	{
		auto touch = item;
